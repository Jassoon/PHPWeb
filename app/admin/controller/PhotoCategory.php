...
class PhotoCategory extends Base
{
    const TABLE = 'photo_category';

    function indexAction()
    {
        $this->assign('title', '类别列表_相册类别');
        $db = D(self::TABLE);
    ...
    function updateAction()
    {
        $this->method(IS_POST);
        $this->param(isset($_POST['submit']));
        D(self::TABLE)->update($_POST) ? jump('更新成功', -2) : alert('更新失败');
    }
