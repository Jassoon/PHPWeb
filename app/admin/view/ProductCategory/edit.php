<div class="tab-bar">
    <ul class="tab">
        ...
        <?php } ?>
        <li class="tab-item"><a href="<?php echo url('ProductCategory'); ?>">类别列表</a></li>
        <li class="tab-item current"><a href="<?php echo SELF_URL; ?>">编辑类别</a></li>
    </ul>
    <ul class="context">
        <li><?php if($prev){ ?><a href="<?php echo url('ProductCategory-edit', $prev['id']); ?>" data-target="replace">上一条</a><?php } ?></li>
        <li><?php if($next){ ?><a href="<?php echo url('ProductCategory-edit', $next['id']); ?>" data-target="replace">下一条</a><?php } ?></li>
    </ul>
</div>
<div class="content">
    <form action="<?php echo url('ProductCategory-update'); ?>" method="post">
        <table class="edit">
            <tr>
                <td align="right" width="60">名称：</td>
                <td><input name="cat_name" type="text"  value="<?php echo $data['cat_name'];?>" size="35" autofocus required title="请输入类别名称"></td>
            </tr>
            <tr>
...
<script src="<?php js('admin/js/productCategory'); ?>"></script>